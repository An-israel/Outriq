...
router.get('/summary', (req, res) => {
  ...
  const activeProducts = getOne('SELECT COUNT(*) as c FROM products WHERE user_id = ? AND status = ?', [uid, 'active'])?.c || 0
  const totalSignals   = getOne('SELECT COALESCE(SUM(signals_count),0) as c FROM products WHERE user_id = ?', [uid])?.c || 0
  const totalActions   = getOne('SELECT COALESCE(SUM(actions_count),0) as c FROM products WHERE user_id = ?', [uid])?.c || 0
  const totalLeads     = getOne('SELECT COALESCE(SUM(leads_count),0) as c FROM products WHERE user_id = ?', [uid])?.c || 0
  const costPerProduct = 4.15

  res.json({
    activeProducts,
    signalsToday: getOne(`SELECT COUNT(*) as c FROM signals s JOIN products p ON p.id = s.product_id
                          WHERE p.user_id = ? AND date(s.created_at) = date('now')`, [uid])?.c || 0,
    signalsChange: '+18%',
...
router.get('/products', (req, res) => {
  const products = getAll(`SELECT id, name, status, match_score, signals_count, actions_count,
